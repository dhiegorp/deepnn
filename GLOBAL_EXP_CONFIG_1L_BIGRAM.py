...
def get_ae_callbacks(network_name):
	ae_callbacks = [
		...
		TensorBoard(log_dir=GLOBAL['tensorflow_dir'] + network_name , histogram_freq=1, write_graph=False)	
	]
	return ae_callbacks
...
def get_mlp_callbacks(network_name):
	...
	mlp_callbacks = [
		...
		TensorBoard(log_dir=GLOBAL['tensorflow_dir'] + network_name + '_mlp', histogram_freq=1, write_graph=False)	
	]
