...
class CSVDatasetLoader:
	def __init__(self, dspath, name_format, resolve_names=True):
		self.__dspath = dspath
		self.__name_format = name_format
		self.__resolve_names = resolve_names
		self.__resolve_file_names()

	def __resolve_file_names(self):
		if self.__resolve_names:
			self.__train_path = ( 
				self.__dspath + self.__name_format + '_train_feat.csv',
				self.__dspath + self.__name_format + '_train_target.csv' 
				)
			self.__test_path = (
				self.__dspath + self.__name_format + '_test_feat.csv',
				self.__dspath + self.__name_format + '_test_target.csv'
				)
	def __load_csv(self):
		self.__trainx = np.loadtxt(self.__train_path[0], delimiter=',')
		self.__trainy = np.loadtxt(self.__train_path[1], delimiter=',')
		self.__testx = np.loadtxt(self.__test_path[0], delimiter=',')
		self.__testy = np.loadtxt(self.__test_path[1], delimiter=',') 

	def __call__(self):
		self.__load_csv()
		return self.__trainx, self.__trainy, self.__testx, self.__testy
